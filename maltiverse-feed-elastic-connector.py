...
parser.add_argument('--elastic_host', dest='elastic_host', default="localhost",
                    help="Specifies elastic database destination hostname. Default 'localhost'")
parser.add_argument('--elastic_port', dest='elastic_port', default=9200,
                    help="Specifies elastic database destination hostname port. Default '9200'")
parser.add_argument('--elastic_username', dest='elastic_username', default=None,
                    help='Specifies elastic database username.')
parser.add_argument('--elastic_password', dest='elastic_password', default=None,
                    help='Specifies elastic database password.')
parser.add_argument('--elastic_index', dest='elastic_index', default="maltiverse",
                    help='Specifies elastic database index.')
parser.add_argument('--verbose', dest='verbose', action="store_true", default=False,
                    help='Shows extra information during ingestion')
parser.add_argument('--delete_old', dest='delete_old', action="store_true", default=True,
            ...
            if insert:
                ...
                if res['result'] == 'created':
                    ...
                    if arguments.verbose:
                        print("Inserted: " + element.get('ip_addr') + " - " + bl['description'] +  " - " + bl['source'])
                if res['result'] == 'updated':
                    COUNT_IP_UPDATED += 1
                    if arguments.verbose:
                        print("Updated: " + element.get('ip_addr') + " - " + bl['description'] +  " - " + bl['source'])
            else:
                COUNT_IP_SKIPPED += 1
        ...
        for bl in element['blacklist']:
            ...
            if arguments.maltiverse_range:
                ...
                if 'hits' in response and response['hits']['total']['value'] == 1:
                    ...
                    if 'hits' in response and response['hits']['total']['value'] > 1:
                        print("WARING: " + str(response['hits']['total']['value']) + " elements found")
            ...
            if insert:
                ...
                if res['result'] == 'created':
                    ...
                    if arguments.verbose:
                        print("Inserted: " + element.get('hostname') + " - " + bl['description'] +  " - " + bl['source'])
                if res['result'] == 'updated':
                    COUNT_HOSTNAME_UPDATED += 1
                    if arguments.verbose:
                        print("Updated: " + element.get('hostname') + " - " + bl['description'] +  " - " + bl['source'])
            else:
        ...
        for bl in element['blacklist']:
            ...
            if insert:
                ...
                if res['result'] == 'created':
                    COUNT_URL_CREATED += 1
                    if arguments.verbose:
                        print("Inserted: " + element.get('url') + " - " + bl['description'] +  " - " + bl['source'])
                if res['result'] == 'updated':
                    COUNT_URL_UPDATED += 1
                    if arguments.verbose:
                        print("Updated: " + element.get('url') + " - " + bl['description'] +  " - " + bl['source'])
            else:
                COUNT_HOSTNAME_SKIPPED += 1
...
if arguments.delete_old:
    ...
    response_delete = es.delete_by_query(index=arguments.elastic_index, conflicts="proceed", body={"query": query})
